...
//     // 🔹 School Header
//     doc.fontSize(18).text(metadata.schoolName, { align: "center", underline: true });
//     doc.moveDown();
//     doc.fontSize(12).text(`${metadata.schoolLocation} | ${metadata.schoolContact}`, { align: "center" });
//     doc.text(`Term: ${metadata.term} | Year: ${metadata.year} | Exam: ${metadata.examType}`, { align: "center" });
//     doc.moveDown(2);

//     reportForms.forEach((report, idx) => {
//       if (idx > 0) doc.addPage();

//       // 🔹 Student Info
//       doc.fontSize(14).text(`Name: ${report.name}`, { continued: true }).text(`   ADM No: ${report.admNo}`, { continued: true }).text(`   Class: ${report.class}`);
//       doc.moveDown();
...
//         doc.text(
//           `${s.learningArea}: ${s.total ?? "-"} | Grade: ${s.grade ?? "-"} | Level: ${s.level ?? "-"} | Remark: ${s.remark ?? "-"}`
//         );
//       });
...
//       // 🔹 Summary
//       doc.text(`Mean Score: ${report.meanScore ?? "-"}`);
//       doc.text(`Grade: ${report.grade ?? "-"}`);
//       doc.text(`Level: ${report.level ?? "-"}`);
//       doc.text(`Position: ${report.position ?? "-"}`);
//       doc.moveDown();

//       // 🔹 Comments
//       doc.text(`Class Teacher Comment: ${report.classTeacherComment}`);
//       doc.text(`Principal Comment: ${report.principalComment}`);
//       doc.moveDown();

//       // 🔹 Footer
//       doc.fontSize(10).text(`Generated by ${metadata.schoolName}`, { align: "center" });
//     });
